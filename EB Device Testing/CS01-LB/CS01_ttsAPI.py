...
def downlink(frm_payload):  
  ...
  payload = requests.post(f'{base}/as/applications/temp-sensor/devices/current-sensor/down/push', json=data,  headers=headers)
...
def get_uplinks(num):
  ...
  earlier_time = current_time - datetime.timedelta(minutes=2)
  ...
  payload = requests.get(f'{base}/as/applications/temp-sensor/devices/current-sensor/packages/storage/uplink_message', headers=headers, params=params)

  ...
  switch (2) {
    case 2:
      ...
      if (poll_message_status === 0) {
        if (bytes.length == 11) {
          let decodedMessage = { extraData: {} };
          //LHT65 has battery in every message
          if (data?.BatV) {
            decodedMessage.extraData.battery = {
              volts: data?.BatV,
          ...
          if (data?.Ext_sensor === "Temperature Sensor") {
            decodedMessage.temp =
              Math.round((data?.TempC_DS * 1.8 + 32) * 100) / 100;
            decodedMessage.internalTemp =
              Math.round((data?.TempC_SHT * 1.8 + 32) * 100) / 100;
          } else {
            decodedMessage.temp =
              Math.round((data?.TempC_SHT * 1.8 + 32) * 100) / 100;
          }

          decodedMessage.humidity =
            Math.round(data?.Hum_SHT * 100) / 100;
...
console.log(decodeUplink({bytes: [0xCB, 0xA8, 0x08, 0x5E, 0x02, 0x65, 0x01, 0x08, 0x4D, 0x7F, 0xFF]}))